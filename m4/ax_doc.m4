...
dnl  + input vars: DOXY_DEFINES, DOXY_TAGPKGS
dnl  + AC_SUBST vars: DOXYGEN, DOXY_FILTER, DOXY_INPUT_FILTER, DOXY_DEFINES, DOXY_TAGFILES
...
if test "$doc_formats" != "none" ; then
  ...
  dnl AC_MSG_NOTICE([setting DOXYGEN=$DOXYGEN])
  ##
  ...
  ##
  AC_ARG_VAR(DOXY_FILTER,[doxygen input filter; "no" or empty to disable])
  ...
  dnl AC_MSG_NOTICE([setting DOXY_FILTER=$DOXY_FILTER])
  ##
  if test "$DOXY_FILTER" != "no"; then
